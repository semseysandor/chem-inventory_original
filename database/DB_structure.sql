...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_hungarian_ci COMMENT='Drug products' AUTO_INCREMENT=427 ;
...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_hungarian_ci COMMENT='API literature DB' AUTO_INCREMENT=3 ;
...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_hungarian_ci COMMENT='PK attributes' AUTO_INCREMENT=18 ;
...
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_hungarian_ci COMMENT='PK data' AUTO_INCREMENT=499 ;
...
CREATE TABLE IF NOT EXISTS `api_summary` (
  ...
  `last_mod_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Last modified on',
  PRIMARY KEY (`api_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_hungarian_ci COMMENT='API DB' AUTO_INCREMENT=502 ;
...
--
-- Table structure for table `leltar_solvent`
--

DROP TABLE IF EXISTS `leltar_solvent`;
CREATE TABLE IF NOT EXISTS `leltar_solvent` (
  `solvent_id` int(11) NOT NULL AUTO_INCREMENT COMMENT 'Solvent ID - PRIMARY KEY',
  `name` varchar(127) COLLATE utf8_hungarian_ci NOT NULL COMMENT 'Solvent name',
  `type` varchar(127) COLLATE utf8_hungarian_ci NOT NULL COMMENT 'Solvent type (HPLC, regular)',
  `volume` double NOT NULL COMMENT 'Volume per unit (L)',
  `unit` int(11) NOT NULL COMMENT 'Inventory units',
  `last_mod_by` varchar(127) COLLATE utf8_hungarian_ci NOT NULL COMMENT 'Last modified by user',
  `last_mod_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Last modified on',
  PRIMARY KEY (`solvent_id`)
) ENGINE=InnoDB  DEFAULT CHARSET=utf8 COLLATE=utf8_hungarian_ci COMMENT='Solvent inventory' AUTO_INCREMENT=12 ;

--
-- Triggers `leltar_solvent`
--
DROP TRIGGER IF EXISTS `leltar_solvent_before_insert`;
DELIMITER //
CREATE TRIGGER `leltar_solvent_before_insert` BEFORE INSERT ON `leltar_solvent`
 FOR EACH ROW BEGIN
  SET @user = SUBSTRING_INDEX(USER(),'@',1);
...
//
DELIMITER ;
DROP TRIGGER IF EXISTS `leltar_solvent_before_update`;
DELIMITER //
CREATE TRIGGER `leltar_solvent_before_update` BEFORE UPDATE ON `leltar_solvent`
 FOR EACH ROW BEGIN
  SET @user = SUBSTRING_INDEX(USER(),'@',1);
...
//
DELIMITER ;
DROP TRIGGER IF EXISTS `leltar_solvent_insert`;
DELIMITER //
CREATE TRIGGER `leltar_solvent_insert` AFTER INSERT ON `leltar_solvent`
 FOR EACH ROW BEGIN
...
/* Insert summary row */
INSERT INTO inventory.changelog_summary (table_name, record_id, action, modified_by, modified_on)
  VALUES (table_n, NEW.solvent_id, act, @user, CURRENT_TIMESTAMP);
...
/* solvent.type */
INSERT INTO inventory.changelog_detail (summary_id, column_name, new_value)
  VALUES (@sum_id, 'type', NEW.type);

/* solvent.volume */
INSERT INTO inventory.changelog_detail (summary_id, column_name, new_value)
  VALUES (@sum_id, 'volume', NEW.volume);

/* solvent.unit */
INSERT INTO inventory.changelog_detail (summary_id, column_name, new_value)
  VALUES (@sum_id, 'unit', NEW.unit);

END
//
DELIMITER ;
DROP TRIGGER IF EXISTS `leltar_solvent_update`;
DELIMITER //
CREATE TRIGGER `leltar_solvent_update` AFTER UPDATE ON `leltar_solvent`
 FOR EACH ROW BEGIN
...
INSERT INTO inventory.changelog_detail (summary_id, column_name, old_value, new_value)
  VALUES (@sum_id, 'type', OLD.type, NEW.type);
END IF;

/* solvent.volume */
IF (NEW.volume <> OLD.volume) THEN
INSERT INTO inventory.changelog_detail (summary_id, column_name, old_value, new_value)
  VALUES (@sum_id, 'volume', OLD.volume, NEW.volume);
END IF;

/* solvent.unit */
IF (NEW.unit <> OLD.unit) THEN
INSERT INTO inventory.changelog_detail (summary_id, column_name, old_value, new_value)
  VALUES (@sum_id, 'unit', OLD.unit, NEW.unit);
END IF;
...
CREATE TABLE IF NOT EXISTS `main_users` (
  ...
  `right_level_solvent` tinyint(4) NOT NULL DEFAULT '0' COMMENT 'Right Level for Solvents',
  `last_mod_by` varchar(127) COLLATE utf8_hungarian_ci NOT NULL COMMENT 'Last modified by user',
  `last_mod_time` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT 'Last modified on',
  PRIMARY KEY (`user_id`)
...
/* users.right_level_solvent */
INSERT INTO inventory.changelog_detail (summary_id, column_name, new_value)
  VALUES (@sum_id, 'right_level_solvent', NEW.right_level_solvent);
...
/* users.right_level_solvent */
IF (NEW.right_level_solvent <> OLD.right_level_solvent) THEN
INSERT INTO inventory.changelog_detail (summary_id, column_name, old_value, new_value)
  VALUES (@sum_id, 'right_level_solvent', OLD.right_level_solvent, NEW.right_level_solvent);
END IF;
